...
export const useSSM = () => {
  ...
  const getParameterValue = useCallback(async (name: string) => {
    try {
  ...
  const putParameter = useCallback(async (name: string, value: string, type: "String" | "StringList" | "SecureString", overwrite = true) => {
    try {
