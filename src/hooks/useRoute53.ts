...
export const useRoute53 = () => {
  ...
  const fetchRecordSets = useCallback(async (hostedZoneId: string) => {
    try {
  ...
  const createHostedZone = useCallback(async (name: string) => {
    try {
      await route53Client.send(new CreateHostedZoneCommand({
        Name: name,
  ...
  const deleteHostedZone = useCallback(async (hostedZoneId: string) => {
    try {
