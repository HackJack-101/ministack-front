...
export const useCloudWatchLogs = () => {
  ...
  const fetchLogStreams = useCallback(async (logGroupName: string) => {
    try {
      const response = await cwLogsClient.send(new DescribeLogStreamsCommand({ logGroupName, orderBy: "LastEventTime", descending: true }));
  ...
  const fetchLogEvents = useCallback(async (logGroupName: string, logStreamName: string) => {
    try {
      const response = await cwLogsClient.send(new GetLogEventsCommand({ logGroupName, logStreamName, startFromHead: false }));
  ...
  const filterLogEvents = useCallback(async (logGroupName: string, filterPattern?: string) => {
    try {
  ...
  const deleteLogGroup = useCallback(async (logGroupName: string) => {
    try {
  ...
  const createLogGroup = useCallback(async (logGroupName: string) => {
    try {
