...
const SNS: React.FC = () => {
  ...
  const fetchSubscriptions = useCallback(async (topicArn: string) => {
  ...
  useEffect(() => { if (selectedTopicArn) fetchSubscriptions(selectedTopicArn); }, [selectedTopicArn, fetchSubscriptions]);

  const handleDeleteTopic = (topicArn: string) => {
    ...
    confirm({
      ...
      action: async () => {
        try {
          ...
          if (selectedTopicArn === topicArn) setSelectedTopicArn(null);
  ...
  const handlePublish = async (e: React.FormEvent) => {
    ...
    try {
      await snsClient.send(new PublishCommand({ TopicArn: selectedTopicArn, Message: publishMessage, Subject: publishSubject || undefined }));
      setPublishMessage("");
      setPublishSubject("");
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    if (!selectedTopicArn) return;
    try {
      await snsClient.send(new SubscribeCommand({ TopicArn: selectedTopicArn, Protocol: newSubProtocol, Endpoint: newSubEndpoint }));
      setNewSubEndpoint("");
      ...
      {!selectedTopicArn ? (
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-medium text-rose-500">{getTopicName(selectedTopicArn)}</span>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-surface-card rounded-card border border-border-subtle p-4">
                ...
                <form onSubmit={handlePublish} className="space-y-3">
                  ...
                  <TextArea label="Message" required rows={4} accentColor="rose"
                    placeholder='{"key": "value"} or just text' value={publishMessage}
                    onChange={(e) => setPublishMessage(e.target.value)} />
                  <Button type="submit" variant="rose" size="sm" className="w-full !justify-center"
                  ...
                  <div className="p-4 bg-rose-500/[0.03] border-b border-rose-500/10">
                    <form onSubmit={handleSubscribe} className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div className="space-y-1.5">
                        ...
                        <select className={selectClass} value={newSubProtocol} onChange={(e) => setNewSubProtocol(e.target.value)}>
                      ...
                      <div className="md:col-span-2 space-y-1.5">
                        ...
                        <div className="flex gap-2">
                          <input type="text" required
                            ...
                            value={newSubEndpoint} onChange={(e) => setNewSubEndpoint(e.target.value)} />
                          <Button type="submit" variant="rose" size="sm">Subscribe</Button>
                          <Button type="button" variant="ghost" size="sm" onClick={() => setShowSubscribe(false)}>Cancel</Button>
                        </div>
                      </div>
                    </form>
                  </div>
